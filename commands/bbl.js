...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('bbl')
        .setDescription('Manually manage BBL roles.')
        .addSubcommand(subcommand =>
            subcommand
                .setName('add')
                .setDescription('Add a BBL team.')
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName('remove')
                .setDescription('Remove a BBL team.')
        ),
    async execute(interaction) {
        ...
        if (interaction.options._subcommand === 'remove') {
            row
                .addComponents(
                    new MessageSelectMenu()
                        .setCustomId('bbl-remove-selection')
                        .setPlaceholder('No team selected')
                        .addOptions([
                            {
                                label: 'All',
                                description: 'Remove all BBL teams',
                                value: 'All'
                            }
                    ])
            );
            ...
            interaction.member.roles.cache.each(role => {
                ...
                if (bblNames.includes(strippedRoleName)) {
                    row.components[0]
                        .addOptions([
                            {
                                label: role.name,
            ...
            await interaction.reply({ content: 'Select BBL team(s) to remove:', components: [row], ephemeral: true });
        }
        else if (interaction.options._subcommand === 'add') {
            row
                .addComponents(
                    new MessageSelectMenu()
                        .setCustomId('bbl-add-selection')
                        .setPlaceholder('No team selected')
                );
            ...
            if (row.components[0].options.length === 0) {
                const embed = new MessageEmbed()
                    .setColor(0xe24540)
                    .setDescription('No BBL teams available to add.')
                ...
                await interaction.reply({ content: 'Select BBL team(s) to add:', components: [row], ephemeral: true })
            }
        }
    },
};